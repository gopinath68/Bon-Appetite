...
server.get('/receipes', (req, res) => {
  ...
  if (req.query.q) {
    ...
    data = data.filter(r =>
      Object.values(r).some(v => {
        ...
        if (Array.isArray(v)) return v.some(i => String(i).toLowerCase().includes(q));
        return false;
      })
    );
  }
  ...
  if (req.query.name_like) {
    ...
    data = data.filter(r => String(r.name || '').toLowerCase().includes(needle));
  }

  // category exact(s) ?category=Dessert  OR ?category=Dessert&category=Chicken
  if (req.query.category) {
    const qs = Array.isArray(req.query.category) ? req.query.category : [req.query.category];
  ...
  if (req.query.category_like) {
    const qs = Array.isArray(req.query.category_like) ? req.query.category_like : [req.query.category_like];
    const lowerQs = qs.map(s => String(s).toLowerCase());
    data = data.filter(r => {
      if (!Array.isArray(r.category)) return false;
      const lowerCats = r.category.map(c => String(c).toLowerCase());
